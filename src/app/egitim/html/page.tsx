...
import htmlLessons from "./ders";

export default function HtmlEgitimPage() {
  const { theme, toggleTheme } = useTheme();
  const [aktifDers, setAktifDers] = useState(htmlLessons[0]);
    ...
    >
      ...
      <div className="flex flex-1 flex-col md:flex-row">
        ...
        <aside className="hidden md:flex w-72 bg-white dark:bg-gray-800 p-6 flex-col shadow-lg">
          <h2 className="text-xl font-bold mb-4">HTML Eğitimleri</h2>
          <div className="flex flex-col gap-2 overflow-y-auto">
            {htmlLessons.map((ders) => (
              <Button
                key={ders.id}
                variant={
                  aktifDers.id === ders.id ? "default" : "outline"
                }
