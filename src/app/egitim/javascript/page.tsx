...
import jsLessons from "./ders"; // jsLessons dosyanı buraya import et

export default function JsEgitimPage() {
    const { theme, toggleTheme } = useTheme();
    const [aktifDers, setAktifDers] = useState(jsLessons[0]);
    const [menuAcik, setMenuAcik] = useState(false);

    return (
        <div
            className={`min-h-screen flex flex-col ${theme === "light" ? "bg-gray-100 text-gray-900" : "bg-gray-900 text-white"
                }`}
        >
            <Header />
            <title>TRC - JavaScript Eğitimleri</title>

            <div className="flex flex-1 flex-col md:flex-row">
                {/* 🖥️ Sol Menü */}
                <aside className="hidden md:flex w-72 bg-white dark:bg-gray-800 p-6 flex-col shadow-lg">
                    <h2 className="text-xl font-bold mb-4">JavaScript Eğitimleri</h2>
                            ...
                            >
                                {jsLessons.map((ders) => (
                                    <div
                                        ...
                                        className={`p-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 ${aktifDers.id === ders.id ? "bg-gray-200 dark:bg-gray-700 font-semibold" : ""
                                            }`}
                                    >
                                        {ders.title}
                                    </div>
